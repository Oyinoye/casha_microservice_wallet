...
import BillerUpdatePage from './biller-update.page-object';

const expect = chai.expect;
export class BillerDeleteDialog {
  deleteModal = element(by.className('modal'));
  private dialogTitle: ElementFinder = element(by.id('walletApp.biller.delete.question'));
...
export default class BillerComponentsPage {
  ...
  title: ElementFinder = element(by.id('biller-heading'));
  ...
  async goToPage(navBarPage: NavBarPage) {
    await navBarPage.getEntityPage('biller');
  ...
  async deleteBiller() {
    ...
    expect(await billerDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/walletApp.biller.delete.question/);
